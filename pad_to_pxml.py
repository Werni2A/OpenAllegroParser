...
def create_scr_file(path_scr: str, path_pxml: str, version: Optional[str] = None) -> None:
...
def export_pad_to_xml(path_pad: str, path_pxml: str) -> None:
    ...
    if not os.path.isfile(path_padstack_editor):
        print(f'Padstack editor was not found at {path_padstack_editor}, adjust this path!')
        sys.exit(-1)
...
parser.add_argument('--pxml_suffix',
...
if __name__ == '__main__':
    ...
    for path_pad in path_pads:
        ...
        if not overwrite and os.path.exists(path_pxml):
            print(f'pxml already exists, skipping export from `{path_pad}` to `{path_pxml}`.')
            continue
