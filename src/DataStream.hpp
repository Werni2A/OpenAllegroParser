...
public:
    ...
    // @todo add parameter to discard padding bytes.
    /**
     * @brief Read string that is zero terminated the end of the string
     *        is additionally padded to a 4 byte boundary.
     *
     * @return std::string Read text.
     */
    std::string readStrZeroTerm4BytePad();

    // @todo add parameter to discard padding bytes.
    /**
     * @brief Read string that is zero terminated and the string
     *        is padded until the block size is reached.
     *
     * @param aBlockSize Size to pad to.
     * @return std::string Read text.
     */
     ...
     *
     * @param aExpectedData Bytes that are exptected.
     * @param aComment Comment in case the assumption failed.
     */
    void assumeData(const std::vector<uint8_t>& aExpectedData, const std::string& aComment = "");

    /**
     * @brief Assume that only zero bytes will be read.
     *
     * @param aLen Byte length to read.
     * @param aComment Comment in case the assumption failed.
     */
    void assumeZero(size_t aLen, const std::string& aComment = "");
};
